...
describe('管理端路由页面', () => {
  ...
  it('新建项目保存后使用最新排序版本立即公开', async () => {
    ...
    const editorForm = wrapper.get('form.editor-form.project-editor-form')
    expect(editorForm.find(':scope > .editor-actions').exists()).toBe(true)
    const formGrid = editorForm.find(':scope > .form-grid')
    expect(formGrid.find('#project-name').exists()).toBe(true)
    expect(formGrid.find('#project-github').exists()).toBe(true)
    expect(formGrid.find('#project-image').exists()).toBe(true)
    expect(formGrid.find('.field.upload').exists()).toBe(true)
    expect(formGrid.get('#project-github').attributes('aria-describedby')).toBe('project-github-hint')
    expect(formGrid.get('#project-github-hint').text()).toBe('隐藏项目可暂不填写；公开时必须是公开的 github.com 仓库地址。')
    expect(editorForm.get('.editor-actions > .button').classes()).toContain('button--small')
    expect(editorForm.get('.editor-actions button[type="submit"]').classes()).toContain('button--small')
    ...
    expect(wrapper.find('.image-upload-preview img.upload__preview').exists()).toBe(true)
  ...
  it('项目公开和隐藏使用完整请求体，并在状态变更后同步排序版本', async () => {
    ...
    await flushPromises()
    expect(wrapper.find('.heading-row .status').exists()).toBe(false)
    expect(wrapper.get('.heading-links .status').text()).toBe('隐藏')
    expect(wrapper.get('.heading-links a.text-link').text()).toBe('返回项目管理')
    expect(wrapper.get('.editor-actions button[type="submit"]').classes()).toContain('button--small')
    expect(wrapper.get('.editor-actions button.button--primary').classes()).toContain('button--small')
    await wrapper.get('button.button--primary').trigger('click')
    await flushPromises()
